...
function CourseCurriculum() {
    ...
    function handlePreviewChange(currentValue, currentIndex) {
        let cpyCourseCurriculumFormData = [...courseCurriculumFormData]
        cpyCourseCurriculumFormData[currentIndex] = {
            ...cpyCourseCurriculumFormData[currentIndex],
    ...
    async function handleMediaBulkUpload(event) {
        ...
        try {
            ...
            if(response.success){
                let cpyCourseCurriculumFormData = areAllCourseCurriculumFormDataObjectEmpty(courseCurriculumFormData) ? [] :  [...courseCurriculumFormData]

                cpyCourseCurriculumFormData = [...cpyCourseCurriculumFormData, ...response.data.map((item, index) =>({
                    videoUrl: item?.url,
    ...
    return (
        <Card>
            ...
            <CardContent>
                <Button onClick={handleNewLecture} disabled={!isCourseCurriculumFormDataValid() || mediaUploadProgress}>
                    Add Lecture
                </Button>
                {
                ...
                <div className="mt-4 space-y-4">
                    {
                        courseCurriculumFormData.map((curriculumItem, index) => <div key={index} className='border p-5 rounded-md'>
                            <div className="flex gap-5 items-center">
                                <h3 className="font-semibold">Lecture {index + 1}</h3>

                                <Input name={`title-${index + 1}`} placeholder="Enter a lecture title" className="max-w-96" onChange={(event) => handleCourseTitleChange(event, index)} value={courseCurriculumFormData[index]?.title} />

                                <div className="flex items-center space-x-2">
                                    <Switch onCheckedChange={(value) => handlePreviewChange(value, index)} checked={courseCurriculumFormData[index]?.freePreview} id={`freePreview-${index + 1}`} />
                            ...
                            <div className='mt-6'>
                                {
                                    courseCurriculumFormData[index]?.videoUrl ? <div className="flex gap-3">
                                        ...
                                        <Button onClick={() =>handleReplaceVideo(index)}>Replace</Button>
                                        <Button className="bg-red-900" onClick={() =>handleDeleteLecture(index)}>Delete Lecture</Button>
                                    </div> : <Input type="file" accept="video/*" className="mb-4" onChange={event => handleSingleLectureUpload(event, index)} />
                                }
