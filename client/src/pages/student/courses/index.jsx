                    ...
                    <div className="">
                        {
                            Object.keys(filterOptions).map(keyItem => (<div className='p-4 border-b' key={keyItem}>
                                ...
                                <div className="grid gap-2 mt-2">
                                    {
                                        filterOptions[keyItem].map((option) =>(<Label key={option?.id} className="flex font-medium items-center gap-3">
                                            <Checkbox checked={filters && Object.keys(filters).length > 0 && filters[keyItem] && filters[keyItem].indexOf(option?.id) > -1} onCheckedChange={()=> handleFilterOnChange(keyItem, option)}/>
                                            {option?.label}
                                        </Label>))
                                    }
                                </div>
                            ...
                            studentViewCoursesList && studentViewCoursesList.length > 0 ? studentViewCoursesList.map(courseItem => (<Card key={courseItem?._id} className="cursor-pointer" onClick={() => handleCourseNavigate(courseItem?._id)}>
                                <CardContent className="flex gap-4 p-4">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-sm text-gray-600">Created By <span className='font-bold'>{courseItem?.instructorName}</span></p>

                                        <p className="text-[16px] text-gray-600 mt-3 mb-2">{`${courseItem?.curriculum.length} ${courseItem?.curriculum.length <=1 ? "Lecture" : "Lectures"} - ${courseItem?.level.toUpperCase()} Level`}</p>

                                        <p className="font-bold text-lg">${courseItem?.pricing}</p>
                                    </div>

                                </CardContent>
                            </Card>)) : (loadingState ? <Skeleton /> : <h1 className="font-extrabold text-4xl">No Courses Found</h1>)
                        }
                    </div>
                </main>
            </div>
